...
namespace RelicsOfTheFallen.Networking.AbilitySystem
{
    ...
    public class NetworkAbilitySystemComponent : NetworkBehaviour
    {
        ...
        private readonly int predictionBufferDuration = 6_000;

        // / <summary> The asc to be synced.  </summary>
        public AbilitySystemComponent asc;
        public static GenericDictionary<uint, NetworkAbilitySystemComponent> ascmDictionary = new();
        public GenericDictionary<uint, NetworkAbilitySystemComponent> ascmDictionaryRef = new();
        public static AbilitySystemComponent localPlayerAsc;
        public static NetworkAbilitySystemComponent localPlayerAscm;
        ...
        void Start()
        {
            name = name + " " + (isLocalPlayer ? "[LocalPlayer]" : "[Srvr]") + " netId=" + netId;

            // ASC and ASCM CACHES AND CACHE DICTIONARY
            if (asc == null) asc = GetComponent<AbilitySystemComponent>();
            if (isLocalPlayer) localPlayerAsc = asc;
            if (isLocalPlayer) localPlayerAscm = this;
            ...
            if (!isClientOnly)
            {
                ...
                {
                    if (x.guid == null || x.guid == "")
                    { // fix in case the instantiated ability doesnt have a guid yet.
                ...
                // {
                //     if (ga.source != asc) activationGUID = null; // Only send the activationGUID to the client that generated it.
                //     RpcOnGameplayAbilityActivated(
                //         ga.guid,
                //         ga.source.GetComponent<NetworkAbilitySystemComponent>().netId,
                //         ga.target.GetComponent<NetworkAbilitySystemComponent>().netId,
                //         activationGUID
                //     );
                // };
                    ...
                    activationGUID) =>
                {
                    ...
                    NetworkAbilitySystemComponent sourceNetwork =
                        ga.source != null
                            ? ga.source.GetComponentInParent<
                                NetworkAbilitySystemComponent>()
                            : null;

                    NetworkAbilitySystemComponent targetNetwork =
                        ga.target != null
                            ? ga.target.GetComponentInParent<
                                NetworkAbilitySystemComponent>()
                            : null;
                    ...
                    {
                        Debug.LogError(
                            $"Cannot replicate ability '{ga.name}': " +
                            $"source or target has no parent " +
                ...
                {
                    if (ga.source != asc) return; // Only send the activation failured to the client that generated it.
                ...
                {
                    if (ge.guid == null || ge.guid == "")
                    { // fix in case the instantiated ge doesnt have a guid yet.
                ...
                {
                    if (ge.durationType == GameplayEffectDurationType.Instant)
                    {// We dont need to add an instant GE to syncDict, it is instant, we just trigger it once.
            ...
            { //  isClientOnly. CLIENT-SIDED TRIGGERS
                ...
                syncGrantedAbilities.OnChange += SynchronizeGA;
                ...
                syncAttributes.OnChange += SynchronizeAttributes;
                ...
                // GE REPLICATION
                syncEffects.OnChange += SynchronizeGE;
        ...
        void SynchronizeGA(SyncDictionary<string, GameplayAbility>.Operation op, string key, GameplayAbility ga)
        {
            ...
            if (!isLocalPlayer)
            { // If this is not the local player (which will use local prediction), remove the GEs, they will come from GE synclist (replication). If we leave them here, they will be applied duplicated when their ability is activated locally.
                ga.effects.Clear();
            }
            switch (op)
            {
        ...
        public void LocalTryActivateAbility(
            GameplayAbility ga,
            string wrongGUID)
        {
            if (ga.source != localPlayerAsc)
            {
                Debug.LogWarning(
                    $"Cannot locally activate ability '{ga.name}': " +
                    "its source ASC does not belong to the local player.",
                    this);
            ...
            NetworkIdentity targetIdentity =
                ga.target != null
                    ? ga.target.GetComponentInParent<NetworkIdentity>()
                    : null;
            ...
            if (targetIdentity == null)
            {
                Debug.LogError(
                    $"Cannot locally activate ability '{ga.name}': " +
                    "the target ASC has no NetworkIdentity in its parent hierarchy.",
                    this);

                return;
            }

            string activationGUID = Guid.NewGuid().ToString();
            ...
            if (predictGameplayAbilityActivations)
            {
                AddAbilityActivityToPredictionBuffer(
                    ga,
                    activationGUID);
            }
        }
        ...
        [Command]
        public void CmdTryActivateAbility(string guid, uint tgtNetId, string activationGUID)
        { // Try to activate the ability on the server
            NetworkAbilitySystemComponent targetASCM = NetworkAbilitySystemComponent.ascmDictionary[tgtNetId];
        ...
        public void RpcOnGameplayAbilityActivated(string guid, uint srcNetId, uint tgtNetId, string activationGUID)
        {
            ...
            NetworkAbilitySystemComponent srcASCM = ascmDictionary[srcNetId];
            NetworkAbilitySystemComponent tgtASCM = ascmDictionary[tgtNetId];
            ...
            if (activationGUID == null || (ga != null && !localAbilityActivationsBuffer.ContainsKey(activationGUID)))
            { // Activation not predicted, activate.
                ga?.CommitAbility(srcASCM.asc, tgtASCM.asc, activationGUID);
            }
            else
            { //  Activation already predicted. Do nothing.
            }
        }
        [ClientRpc]
        public void RpcOnGameplayAbilityDeactivated(string guid, string activationGUID)
        {
            ...
            if (activationGUID == null || ga != null && !localAbilityActivationsBuffer.ContainsKey(activationGUID))
            { // Deactivation not predicted, deactivate.
        ...
        [ClientRpc]
        void RpcGameplayAbilityUndo(string activationGUID, ActivationFailure failure)
        { // If a GA failed to activate on the server, we deactivate it if active AND remove any effect applied by it.
            if (!isClientOnly) return; // Only run this if this is a client only. If it's a host or server, we dont want this code because the changes have been applied already
        ...
        //     // ATTRIBUTE PREDICTION BUFFER
        //     if (localAttributesBuffer.ContainsKey(attributeName) && localAttributesBuffer[attributeName].Count > 0 && localAttributesBuffer[attributeName].Contains(newValue))
        //     {// Dequeue until we found the value already predicted
        ...
        //     }
        //     else
        //     { // If the value received from the server was not predicted, wipe the buffer and assign it immediately. Server conciliation.
        //         if (localAttributesBuffer.ContainsKey(attributeName)) localAttributesBuffer[attributeName].Clear();

        //         asc.attributesDictionary.TryGetValue(attributeName, out GAS.Attribute attribute);
        //         float oldValue = attribute.GetValue();
        //         attribute.baseValue = newValue;
        //         attribute.currentValue = newValue;
    ...
    float callbackValue)
        ...
        IEnumerator SynchronizeAttributesCoroutine(
            SyncDictionary<string, float>.Operation operation,
            string attributeName)
        {
            yield return new WaitForEndOfFrame();

            if (operation == SyncDictionary<string, float>.Operation.OP_REMOVE ||
                operation == SyncDictionary<string, float>.Operation.OP_CLEAR)
            {
                yield break;
            }

            if (!syncAttributes.TryGetValue(
                    attributeName,
            ...
            if (!asc.attributesDictionary.TryGetValue(
                    attributeName,
            ...
            if (localAttributesBuffer.TryGetValue(
                    attributeName,
                    out Queue<float> predictionBuffer) &&
                predictionBuffer.Count > 0 &&
                predictionBuffer.Contains(authoritativeValue))
            {
                while (predictionBuffer.Count > 0)
                {
                    float predictedValue = predictionBuffer.Dequeue();
        ...
        // ATTRIBUTE PREDICTION BUFFER
        void AddAttributeToPredictionBuffer(AttributeName attName, float oldValue, float newValue, GameplayEffect ge)
        ...
        public void SynchronizeGE(SyncDictionary<string, GameplayEffect>.Operation op, string key, GameplayEffect item)
        {
            ...
            {
                // GE PREDICTION BUFFER
                if (localEffectsBuffer.Count > 0 && localEffectsBuffer.ContainsKey(EffectBufferKey(item)))
                { // Do not add GE if already predicted locally
                ...
                { // Add GE if not predicted locally
            ...
            if (op == SyncDictionary<string, GameplayEffect>.Operation.OP_REMOVE)
            {
                ...
                geToRemove ??= asc.appliedGameplayEffects.Find(x => x.applicationGUID == item.applicationGUID && x.name == item.name); //If we couldnt find the guid, try finding by applicationGUID and name.

                if (geToRemove == null)
                { //  If the ge still can't be found, dont try to remove it. It was probably removed by prediction.
                    return;
                }
            }
        }
        ...
        // GE PREDICTION BUFFER
        async void AddEffectApplicationToPredictionBuffer(GameplayEffect ge, AbilitySystemComponent src)
        {// Buffers the sequence of changes to tags. If the sequence is different from sequence of changes receivedfrom server. Clear it and reset to server value.
        ...
        string EffectBufferKey(GameplayEffect ge)
        {// This will make an unique key for the predicted ge, using ge.applicationGUID and its index on the ga.
            ...
            if (index == "-1")
            { // Check if it's a cooldown or cost GE
    ...
    public static class CustomReadWriteFunctions
    {
        ...
        public static void WriteMyAbilitySystemComponent(
            this NetworkWriter writer,
            AbilitySystemComponent value)
        {
            NetworkIdentity networkIdentity =
                value != null
                    ? value.GetComponentInParent<NetworkIdentity>()
                    : null;

            writer.WriteUIntNullable(
        ...
        public static AbilitySystemComponent ReadMyAbilitySystemComponent(this NetworkReader reader)
        {
            ...
            NetworkAbilitySystemComponent.ascmDictionary.TryGetValue((uint)netId, out NetworkAbilitySystemComponent ascm);
            return ascm != null ? ascm.asc : null;
        }
